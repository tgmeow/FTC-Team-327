...
public class Transform {
    ...
    public static void enlarge(Mat img, Size approxSize, boolean integerScale) {
        ...
        if (org.lasarobotics.vision.test.util.MathUtil.equal(scale, 1) || scale < 1) {
            return;
        }
        scale(img, scale);
    }
    ...
    private static double makeScale(Mat img, Size approxSize, boolean maximize, boolean integerScale) {
        ...
        if (org.lasarobotics.vision.test.util.MathUtil.equal(ratio, 1))
            return 1;
        if (integerScale) {
    ...
    public static void resize(Mat img, Size size) {
        int interpolation;
        if (org.lasarobotics.vision.test.util.MathUtil.equal(size.area(), img.size().area()))
            return;
