...
class BeaconScoring {
    ...
    private static double createSubscore(double value, double bestValue, double variance, double bias, boolean ignoreSign) {
        return Math.min(org.lasarobotics.vision.test.util.MathUtil.normalPDFNormalized((ignoreSign ? value - bestValue : Math.max((value - bestValue), 0)) / bestValue,
                variance, 0) * bias, bias);
    }

    List<ScoredContour> scoreContours(List<org.lasarobotics.vision.test.detection.objects.Contour> contours,
    ...
    List<ScoredEllipse> scoreEllipses(List<org.lasarobotics.vision.test.detection.objects.Ellipse> ellipses,
                                      @Nullable Point estimateLocation,
                                      @Nullable Double estimateDistance,
                                      Mat gray) {
        ...
        for (org.lasarobotics.vision.test.detection.objects.Ellipse ellipse : ellipses) {
            ...
            org.lasarobotics.vision.test.detection.objects.Ellipse e = ellipse.scale(0.5); //get the center 50% of data
            double averageColor = e.averageColor(gray, org.lasarobotics.vision.test.util.color.ColorSpace.GRAY).getScalar().val[0];
        ...
        for (ScoredContour contour : contours) {
            ...
            for (ScoredEllipse ellipse : ellipses) {
                if (ellipse.ellipse.isInside(contour.contour) ||
                        (org.lasarobotics.vision.test.util.MathUtil.distance(ellipse.ellipse.center(), contour.contour.center()) <=
    ...
    static class ScoredEllipse extends Scorable {
        org.lasarobotics.vision.test.detection.objects.Ellipse ellipse;

        ScoredEllipse(org.lasarobotics.vision.test.detection.objects.Ellipse ellipse, double score) {
            super(score);
            this.ellipse = ellipse;
        }

        public static List<org.lasarobotics.vision.test.detection.objects.Ellipse> getList(List<ScoredEllipse> scored) {
    ...
    static class ScoredContour extends Scorable {
        org.lasarobotics.vision.test.detection.objects.Contour contour;

        ScoredContour(org.lasarobotics.vision.test.detection.objects.Contour contour, double score) {
            super(score);
            this.contour = contour;
        }

        public static List<org.lasarobotics.vision.test.detection.objects.Contour> getList(List<ScoredContour> scored) {
